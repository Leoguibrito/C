...
void transferir(conta_t *origem, conta_t *destino, double valor) {
    pthread_mutex_t *maior_id;
    pthread_mutex_t *menor_id;
    
    if (origem -> id > destino -> id) {
        maior_id = &origem->mutex;
        menor_id = &destino->mutex;
    } else {
        maior_id = &destino->mutex;
        menor_id = &origem->mutex;
    }
    pthread_mutex_lock(maior_id);
    pthread_mutex_lock(menor_id);

    transferir_unsafe(origem, destino, valor);

    pthread_mutex_unlock(menor_id);
    pthread_mutex_unlock(maior_id);
}
